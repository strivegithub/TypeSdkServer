...
CREATE TABLE `sdk_order` (
  ...
  `ord_amount` float(10,1) DEFAULT NULL COMMENT '渠道商品回调返回金额(非实际支付金额)',
...
) ENGINE=InnoDB AUTO_INCREMENT=34508 DEFAULT CHARSET=utf8 COMMENT='订单记录表';

-- ----------------------------
-- Records of sdk_order
-- ----------------------------
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_sdk_order_update_status`(IN `p_ord_game` CHAR(30), IN `p_ord_channel` CHAR(30), IN `p_ord_cporder` CHAR(100), IN `p_ord_chorder` CHAR(100), IN `p_ord_status` TINYINT(1), IN `p_ord_amount` FLOAT(10))
    NO SQL
IF ISNULL(p_ord_cporder) || LENGTH(TRIM(p_ord_cporder)) < 1
THEN
	SELECT -1;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `p_sdk_order_update_statusAndData`(IN `p_ord_game` CHAR(30),IN `p_ord_channel` CHAR(30),IN `p_ord_cporder` CHAR(100),IN `p_ord_chorder` CHAR(100),IN `p_ord_status` TINYINT(1), IN `p_ord_amount` FLOAT(10),IN `p_ord_data` mediumtext)
IF ISNULL(p_ord_cporder) || LENGTH(TRIM(p_ord_cporder)) < 1
THEN
	SELECT -1;
ELSE
